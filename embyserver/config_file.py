...
users 只有第一次啟動用來建立帳號的那幾個（含明碼密碼，所以設定檔和它的備份要收好）；
之後在網頁新增、修改的帳號、115 登入狀態、網頁建立的 API 金鑰都存在資料庫，不寫進設定檔。
"""
...
def render(config: Config) -> str:
    ...
    lines = [
        ...
        "# 底下 users 只在第一次啟動時用來建立帳號；之後在網頁上改的帳號密碼、115 登入狀態、API 金鑰都存在資料庫，不在這裡。",
        "",
        "server:",
