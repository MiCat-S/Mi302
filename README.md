...
媒體庫裡的劇少了幾集時，可以讓 MoviePilot 去下載補齊。「MoviePilot」分頁的「補全缺集」卡片列出媒體庫裡所有的劇和每一季有幾集，集號有空洞的（例如有第 2、4 集沒有第 3 集）會標出來；可以搜尋劇名、只看某一年的、選每頁顯示幾部（20–200），按「補全」只補那一部，或按「全部補全」把所有的劇都送出去。
...
- 有人在開頭 10 分鐘內（短的集是前 25%，24 分鐘的動畫就是前 6 分鐘）往前跳過 15 秒到 3 分鐘（不是倍速播放），就記成這一集的片頭，從跳走的位置到跳到的位置。只看跳的那一下。
- 在最後 5 分鐘（短的集是最後 25%）停下、切下一集（離結尾還有 20 秒以上），或往前跳了 60 秒以上、跳到結尾，就記成片尾開始的位置。
