...
Mi302 做的只是替那部劇的每一季向 MoviePilot 建一條訂閱。MoviePilot 收到後自己拿 TMDB 的集數對照 Mi302 裡已有的集：已經齊全的會拒絕（「媒体库中已存在」），不會留下訂閱；缺集的建訂閱、開始找資源，下載整理完再通知 Mi302；還在更新的劇會跟著追新集。所以「集號有空洞」只是提示，最後幾集沒下到、整季都沒有的情況 MoviePilot 一樣算得出來。
